...
def init_session(session_key, session_file):
...
def create_and_init_session_background(fingerprint):
    ...
    try:
        ...
        init_session(session_key, session_file)
        warmup_line_count = get_line_count(session_file)
...
def handle_message():
    ...
    if not user:
        if message.startswith("BTN:"):
            ...
            first_context = canned  # Store actual canned text so session knows what was shown
        else:
    ...
    # CASE 2: Waiting for name (user just typed their name, OR sent a follow-up question)
        ...
        if is_question:
            ...
            prefixed_message = (
                ...
                f"Reply naturally, addressing them by name. You can reference what was shown earlier if relevant."
            )
    ...
    # CASE 3: Active / returning user
        ...
        # For active/returning users, the session was already warmed up during button click.
        # No need to re-inject the full INIT_MESSAGE — just send the user's actual message
        # with enough context for the model to respond coherently.
