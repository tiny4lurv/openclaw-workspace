...
def create_and_init_session_background(fingerprint):
    ...
    try:
        ...
        # Step 2: Warm it up with INIT_MESSAGE (no line count tracking yet)
        init_session(session_key)
        # Step 3: Get line count AFTER warm-up to set the warm-up baseline
        warmup_line_count = get_line_count(session_file)
        db.save_session_warmup_lines(fingerprint, warmup_line_count)
        # Step 4: Ping to confirm responsive before marking ready
        try:
            ws = ws_connect()
            req_id = str(uuid.uuid4())[:8]
            ws.send(json.dumps({
                "type": "req", "id": req_id, "method": "sessions.send",
                "params": {"key": session_key, "message": "ping", "idempotencyKey": req_id}
            }))
            ws.close()
        except:
            pass
